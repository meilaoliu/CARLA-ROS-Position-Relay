...
"""
#它的作用是实现车辆的底层控制。该文件接收来自车辆的状态反馈（油门、刹车、转向角度等），
# 并将控制信号发送到Carla车辆上。它通过`CarlaEgoVehicleControl`消息发布控制命令
# 猜测：若启用这个脚本，则Autoware会直接驱动仿真里的车辆，若不启用这个脚本，
# 则Autoware的输出指令会直接输出到实际车辆，Carla中的车辆会同步现实中的车辆状态
